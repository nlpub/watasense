...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:key name="hypernyms" match="/rwn/relations/relation" use="@parent_id"/>
  <xsl:template match="/">
    <xsl:for-each select="/rwn/synsets/synset">
      ...
      <xsl:text>&#9;</xsl:text>
      <!-- count -->
      <xsl:value-of select="count(key('synsets', key('hypernyms', @id)/@child_id))"/>
      <xsl:text>&#9;</xsl:text>
      <!-- hypernyms -->
      <xsl:for-each select="key('synsets', key('hypernyms', @id)/@child_id)">
        <xsl:if test="position() &gt; 1">
          <xsl:text>|</xsl:text>
        </xsl:if>
